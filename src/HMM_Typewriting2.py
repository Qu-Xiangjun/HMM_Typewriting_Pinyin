...
py2hanzi = np.load('../data2/py2hanzi.npy', allow_pickle=True).item()
# dic 汉字：汉字编码  映射表
hanzi_dict = np.load('../data2/hanzi_dict.npy', allow_pickle=True).item()
# array 汉字到汉字的二维表，填充出现次数
hanzi_matrix = np.load('../data2/moving_array.npy', allow_pickle=True)  # 汉字出现次数
# array 汉字编码：汉字出现次数
hanzi_num = np.load('../data2/hanzi_num.npy', allow_pickle=True)  # 汉字出现次数
total_num = sum(hanzi_num)
# 汉字 对应的 拼音 频数 eg：{'了':{'le':5, 'liao':10},'屈':{'qu':5}}
hanzi2pin_dict = np.load('../data2/emission_dic.npy', allow_pickle=True).item()
...
def Typewriting():
    ...
    # 输出的文件
    input_py.close()
    output_hz = open('../data2/output_answer_for_测试集.txt', 'w')
